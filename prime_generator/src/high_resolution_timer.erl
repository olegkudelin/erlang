%% Реализует функции таймера.
%% Если процесс хочет, чтобы его вызывали с определенным интервалом, он передает сюда (add_method)
%% функцию обраного вызова и интервал в микросекундах, через какой надо вызывать ее
%% Таймер кладет такие функции в список и в бесконечном цикле отсматривает этот список,
%% в поисках функции, время вызова которой подошло (check_timer). Как только он находит, то сохраняет
%% текущее время и производит ее вызов
%%
